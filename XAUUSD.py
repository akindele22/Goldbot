...
def consolidation(data8):
    try:
        x = []
        for i in range(data8.shape[0]):
            counter  = 0
            upb = (data8.iloc[i-counter]['high'] + data8.iloc[i-counter+1]['high'] + data8.iloc[i-counter+2]['high']) / 3
            lwb = (data8.iloc[i-counter]['low'] + data8.iloc[i-counter+1]['low'] + data8.iloc[i-counter+2]['low']) / 3
            while counter <= 15:
                if data8.iloc[i] < upb:
                    x.append('pause')
                elif data8.iloc[i] > lwb:
                    x.append('pause')
                counter = counter + 1
                
    except IndexError:
        x.append('not valid') 
    consolidate =  pd.DataFrame(x, columns=['consolidation'])
    consolidate.to_csv('consolidate.csv')
    return consolidate
...
def signa(data8):
    consolidation(data8)
    consolidate = pd.read_csv('consolidate.csv')
    data8 = data8.join(consolidate,how = 'left', lsuffix='_left')
    data8 = data8.dropna()
    data8.drop(labels=['Unnamed: 0'], axis=1)
...
def get_data7(symbol, timeframe,roll_period):
    ...
    return ohlc_data7
def get_data8(symbol, timeframe,roll_period):
    ohlc_data8 = pd.DataFrame(mt.copy_rates_from_pos(symbol,
                                                timeframe,
                                                1,
                                                roll_period))
    #fig = px.line(ohlc_data7, x=ohlc_data7['time'], y=ohlc_data7['close'])
    ohlc_data8['time']=pd.to_datetime(ohlc_data8['time'], unit='s')
    ohlc_data8.to_csv('ohlc_data8.csv')
    return ohlc_data8
...
def trade7(): 
    ...
    while True:
        ...
        timeframe1 = mt.TIMEFRAME_M10
        get_data8(symbol, timeframe1,roll_period)
        ohlc_data8 = pd.read_csv('ohlc_data8.csv')
        data8 = ohlc_data8.copy()
        ...
        if (pred_clf_xau().predict(X_test) == 1).any():
            ...
            direction = 'sell'
        elif data8.iloc['consolidation'] == 'pause':
            direction = 'pass'
